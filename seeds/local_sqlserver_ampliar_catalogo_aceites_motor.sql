...
-- Mismo contenido que ampliar_catalogo_aceites_motor.sql en sintaxis SQL Server.
-- ================================================================

USE cixoil;

INSERT INTO product (name, description, price, viscosity, id_product_brand, id_category, status, created_at, updated_at)
SELECT v.name, v.description, v.price, v.viscosity, v.id_product_brand, 1, 1, GETDATE(), GETDATE()
FROM (
