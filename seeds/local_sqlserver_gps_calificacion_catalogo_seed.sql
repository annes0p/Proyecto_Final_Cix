...
-- Mismo contenido que migracion_gps_calificacion_catalogo.sql pero en
-- sintaxis SQL Server. Ejecutar UNA sola vez, en este orden.
-- ================================================================

USE cixoil;

-- 1) Columnas nuevas en `incident`
ALTER TABLE incident ADD resolved_at DATETIME2 NULL;
ALTER TABLE incident ADD rating INT NULL;

-- 2) Tabla nueva para la ubicacion GPS en vivo del vendedor durante un viaje
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'trip_location')
BEGIN
...
-- 3) Mas modelos de vehiculo (usa marcas/tipos ya existentes)
